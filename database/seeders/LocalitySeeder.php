...
class LocalitySeeder extends Seeder
{
    ...
    {
        $this->countries()
            ->each(fn (Country $country) => $this->counties($country)
    ...
    {
        return Collection::wrap(File::files($this->path([$country->iso_3166_3])))
    ...
    private function localities(Country $country, $county): array
    {
        return (new JsonReader($this->path([$country->iso_3166_3, $county->getFileName()])))->collection()
            ->map(fn ($locality) => Collection::wrap($locality)
    ...
    {
        return Collection::wrap(File::directories($this->path()))
